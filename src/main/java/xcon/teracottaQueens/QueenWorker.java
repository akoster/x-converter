...
public class QueenWorker {
    ...
    private void run() throws InterruptedException {
        ...
        while (true) {
            ...
            // 1 = type progress
            // 2 = type result
            QueenResult queenResult =
                new QueenResult(QueenResult.Type.RESULT,
    ...
    private void reportProgess() {
        ...
        if (now - lastProgressTimeMillis > REPORT_INTERVAL_MILLIS) {
            ...
            progressPercentage =
                (int) Math.round(100.0 * queenRank / queenTotal);
            System.out.println("size=" + size + " q1=" + q1 + " q2=" + q2
                + " result=" + progressPercentage);

            QueenResult queenResult =
                new QueenResult(QueenResult.Type.PROGRESS,
                    currentTask.queenColumn, progressPercentage);
            queenWork.results.add(queenResult);
